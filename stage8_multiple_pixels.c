// stage8_multiple_pixels.c - MANUAL GRADIENTS, MULTIPLE PIXELS
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <string.h>
...
// Render one Gaussian to one pixel (helper)
void render_pixel(
    double* params,
    Camera* cam,
...
// Backward pass for one pixel (accumulate into grad_params)
void backward_pixel(
    double* params,
    Camera* cam,
    double pixel_x, double pixel_y,
    double* target_color,
    double* grad_params  // Accumulate into this!
) {
    // === FORWARD PASS ===
    double gx = params[0], gy = params[1], gz = params[2];
    double gr = params[3], gg = params[4], gb = params[5], ga = params[6];
    ...
    grad_params[3] += dL_dcolor_r * alpha;  // Accumulate!
    grad_params[4] += dL_dcolor_g * alpha;
    grad_params[5] += dL_dcolor_b * alpha;
    
    double dL_dweight = dL_dalpha * ga;
    grad_params[6] += dL_dalpha * weight;
    ...
    grad_params[0] += dL_drel_x;  // Accumulate!
    grad_params[1] += dL_drel_y;
    grad_params[2] += dL_drel_z;
}

int main() {
    printf("=== STAGE 8: Multiple Pixels (%dx%d Patch) ===\n\n", PATCH_SIZE, PATCH_SIZE);
    ...
    printf("Loaded %dx%d patch from center of %dx%d image\n", 
           PATCH_SIZE, PATCH_SIZE, img->width, img->height);
    ...
    // === OPTIMIZATION ===
    printf("Optimizing one Gaussian to fit %d pixels...\n\n", PATCH_SIZE*PATCH_SIZE);
    
    double learning_rate = 0.0005 / (PATCH_SIZE * PATCH_SIZE);  // Scale by num pixels
    int num_iters = 5000;
    ...
    for (int iter = 0; iter < num_iters; iter++) {
        ...
        for (int py = 0; py < PATCH_SIZE; py++) {
            for (int px = 0; px < PATCH_SIZE; px++) {
                ...
                backward_pixel(params, cam, pixel_x, pixel_y, target[py][px], grad);
            }
        }
        ...
        if (iter % 500 == 0) {
            ...
            for (int py = 0; py < PATCH_SIZE; py++) {
                for (int px = 0; px < PATCH_SIZE; px++) {
                    ...
                    render_pixel(params, cam, pixel_x, pixel_y, color);
    ...
    render_pixel(params, cam, center_x, center_y, center_color);
    printf("\nCenter pixel:\n");
    ...
    printf("\n✓✓✓ SUCCESS! One Gaussian fits multiple pixels!\n");
    printf("    Ready for Stage 9 (multiple Gaussians)\n");
